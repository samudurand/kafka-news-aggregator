...
import com.kafka.experiments.shared.{ArticleTweet, AudioTweet, ExcludedTweet, OtherTweet, ToolTweet, Tweet, URLEntity, User, VersionReleaseTweet, VideoTweet}
...
class CategorizerTest extends AnyFlatSpec with Matchers with MockFactory with BeforeAndAfterEach {
  ...
  "A Tweet mentioning with a link containing a number" should "not be identified as a new Version" in {
    (redisService.putWithExpire _).expects(*).returning(false)
    val tweet = goodTweet.copy(Text = "version https://stack.com/34/link")

    categorizer.categorize(tweet) shouldBe a[OtherTweet]
  }

  "A Tweet mentioning a Version with combined words" should "be identified as an announcement about a new Version" in {
    (redisService.putWithExpire _).expects(*).returning(false)
    val tweet = goodTweet.copy(Text = "new version available!")
